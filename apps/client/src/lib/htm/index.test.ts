...
describe("htm", () => {
  ...
  test("single boolean prop", () => {
    expect(html`<a disabled />`).toEqual({
      tag: "a",
      props: { disabled: true },
      children: [],
    });
  ...
  test("slash in the middle of tag name or property name self-closes the element", () => {
    expect(html`<ab/ba prop=value>`).toEqual({
      tag: "ab",
      props: null,
      children: [],
    });
    expect(html`<abba pr/op=value>`).toEqual({
      tag: "abba",
      props: { pr: true },
      children: [],
    });
  ...
  test("prop with multiple static and dynamic values get concatenated as strings", () => {
    ...
    expect(html`<a href="${1}${1}" />`).toEqual({
      tag: "a",
      props: { href: "11" },
      children: [],
    });
    ...
    expect(html`<a href=/before/${"foo"}/>`).toEqual({
      tag: "a",
  ...
  test("spread props", () => {
    ...
    expect(html`<a b=${2} ...${{ c: 3 }}>d: ${4}</a>`).toEqual(
      h("a", { b: 2, c: 3 }, "d: ", 4)
    );
  ...
  test("text child", () => {
    expect(html`<a>foo</a>`).toEqual({
      tag: "a",
      props: null,
      children: ["foo"],
    });
    expect(html`<a>foo bar</a>`).toEqual({
      tag: "a",
      props: null,
      children: ["foo bar"],
    });
  ...
  test("mixed text + dynamic children", () => {
    ...
    expect(html`<a>foo${null}</a>`).toEqual({
      tag: "a",
      props: null,
      children: ["foo", null],
    });
  ...
  test("mixed typed children", () => {
    ...
    expect(html`<a>before<b />after</a>`).toEqual(
  ...
  test("NUL characters are allowed in attribute values", () => {
    ...
    expect(html`<a b=" " c=${"foo"}></a>`).toEqual(
      h("a", { b: "\0", c: "foo" })
    );
  });
  ...
  test("NUL characters are allowed in text", () => {
    ...
    expect(html`<a></a>`).toEqual(h("a", null, "\0", "foo"));
  });

  test("cache key should be unique", () => {
    html`<a b="${"foo"}" />`;
    expect(html`<a b=" " />`).toEqual(h("a", { b: "\0" }));
    expect(html`<a>${""}9aaaaaaaaa${""}</a>`).not.toEqual(
      html`<a>${""}0${""}aaaaaaaaa${""}</a>`
    );
    expect(html`<a>${""}0${""}aaaaaaaa${""}</a>`).not.toEqual(
      html`<a>${""}.8aaaaaaaa${""}</a>`
    );
  });
